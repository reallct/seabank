...
import com.reallct.seabank.exception.AuthException;
...
public class BaseController {
	...
	* @version V1.0
	 ...
	 */
	UserEntity checkLoginForJWT(HttpServletRequest request) throws BaseException {
		UserEntity res = checkLoginForJWTSilence(request);
		if(StringUtil.isNullOrEmpty(res)) {
			throw new AuthException("not Login!", "没有登陆或登陆超时");
		}
		if(StringUtil.isNullOrEmpty(res.getUserId())) {
    ...
    */
    UserEntity checkLoginForJWTSilence(HttpServletRequest request) {
